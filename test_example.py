...
def test_api():
    ...
    response = requests.post(f"{BASE_URL}/configs", json=data, headers=HEADERS)
    print(f"Status: {response.status_code}")
    print(f"Response: {json.dumps(response.json(), indent=2)}\n")

    # 2. READ
    print("2. READ - Reading configuration")
    response = requests.get(f"{BASE_URL}/configs/database_config", headers=HEADERS)
    print(f"Status: {response.status_code}")
    print(f"Response: {json.dumps(response.json(), indent=2)}\n")
    ...
    response = requests.put(f"{BASE_URL}/configs/database_config", json=data, headers=HEADERS)
    print(f"Status: {response.status_code}")
    print(f"Response: {json.dumps(response.json(), indent=2)}\n")
    ...
    response = requests.get(f"{BASE_URL}/configs?category=database", headers=HEADERS)
    print(f"Status: {response.status_code}")
    print(f"Response: {json.dumps(response.json(), indent=2)}\n")

    # 7. DELETE
    print("7. DELETE - Deleting API configuration")
    response = requests.delete(f"{BASE_URL}/configs/api_config", headers=HEADERS)
    print(f"Status: {response.status_code}")
    print(f"Response: {json.dumps(response.json(), indent=2)}\n")
    ...
    response = requests.delete(f"{BASE_URL}/configs/database_config", headers=HEADERS)
    print(f"Status: {response.status_code}")
    print(f"Response: {json.dumps(response.json(), indent=2)}\n")
